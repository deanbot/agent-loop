...
- if NEW_INLINE_COMMENT or NEW_COMMENT: skip own `[executor]` posts; operator (unprefixed) — fix or answer, then for each addressed inline comment reply directly to its thread: `gh api repos/<repo>/pulls/<PR>/comments -X POST -f body="[executor] <response>" -f in_reply_to_id=<id>`; `[reviewer]` — treat as CHANGES_REQUESTED (batch all, fix all, push once, one summary comment); ScheduleWakeup(60s), poll again
